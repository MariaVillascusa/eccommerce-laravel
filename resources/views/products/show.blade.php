                    ...
                    <ul class="slides">
                        @foreach ($product->images as $image)
                            <li data-thumb="{{ Storage::url($image->url) }}">
                                <img dusk="image-product-{{ $loop->index }}" src="{{ Storage::url($image->url) }}" />
                            </li>
                        @endforeach
                    </ul>
                </div>
            ...
            <div>
                ...
                <div class="bg-white rounded-lg shadow-lg mb-6">
                    <div class="flex items-center p-4">
                        ...
                        <div class="ml-4">
                            <p class="text-lg font-semibold text-lime-600">Se hacen envíos solo a la península</p>                             <p>Recíbelo el {{ Date::now()->addDay(7)->locale('es')->format('l j F') }}</p>
                        </div>
                    </div>
                </div>
